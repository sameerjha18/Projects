	...
	public function add_purchase()
	{
		...
		else
		{
			if($this->input->post('isAjax')=='1'):
				...
				$this->form_validation->set_rules('status','Product brand','trim|required|xss_clean');
				...
				$template = "admin";
				$record['sp'] = $this->admin_model->getWhere('supplier',array('sp_status'=>'1'));
		...
		if (empty($adminSession)) {
			...
			if ($this->input->post('isAjax') == '1') :
				$record = $this->input->post();
				   // print_r($record);
				   // exit();
				$this->form_validation->set_rules('update', 'Purchase Date', 'trim|required');
				$this->form_validation->set_rules('upsupplier', 'Purchase Supplier Name', 'trim|required');
				$this->form_validation->set_rules('remark', 'Purchase Remark', 'trim|required');
	...
	public function update_purchasedetails($purchase_id,$pd_id)
	{
		...
		else
		{
			...
			if($this->input->post('isAjax')=='1'):
				...
				$this->form_validation->set_rules('ustatus','Status','trim|required|xss_clean');
