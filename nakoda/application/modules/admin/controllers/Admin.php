...
class Admin extends MX_Controller {
	...
	public function index()
	{
		...
		if($this->input->post('isAjax')=='1')
		{
			...
			if($this->form_validation->run()==FALSE):
				...
				$conditionArray = array('admin_name'=>strtolower($record['username']),'admin_password'=>$record['userpassword']);
				$logindata = $this->admin_model->getWhere('adminmaster',$conditionArray);
		...
		else
		{
			...
			else
			{
				...
				$template = "admin"; //Template Controller function name
				$record['title'] = 'Admin - login';
				$record['viewFile'] = "login"; //Site controller's view file
				$record['module'] = "admin"; //controller name
	...
	public function dashboard()
	{
		...
		{
			$template = "admin";
//			$record['total_customer'] = $this->admin_model->get_count('customer',array('status'=>'0'));
//			$record['total_salesuser'] = $this->admin_model->get_count('salesperson',array('status'=>'0'));
			$record['title'] = 'Admin - Dashboard';
	...
	/*public function deleterecord()
	{
		...
		$table = $this->input->post('tb_data');

		$tb_field = $this->input->post('tb_field');
		$tb_field1 = $this->input->post('tb_field1');
		$condition = array($tb_field=>$id);

		$delete_response = $this->db->update($table,array($tb_field1=>2),$condition);
